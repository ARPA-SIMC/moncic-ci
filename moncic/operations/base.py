    ...
    * /srv/moncic-ci/source/{name} (volatile): sources are mounted here
    ...
    def __init__(
        ...
        *,
        ...
        self.host_root = Path(
            self.enter_context(
                tempfile.TemporaryDirectory(
                    suffix=f"host-root-{self.__class__.__name__}"
                )
            )
        )
        #: Path in the container used as the root of our operation-specific
        #: filesystem tree
        ...
        self.guest_source_path = (
            self.guest_root / "source" / self.source.path.name
        )
        #: Modular units of functionality activated on this operation
        self.plugins: list[
            Callable[[ContainerConfig], ContextManager[None]]
        ] = [
    ...
    def plugin_mount_source(self, config: ContainerConfig) -> Generator[None]:
        ...
        config.configure_workdir(
            ...
            mountpoint=self.guest_source_path.parent,
        )
        yield

    @contextlib.contextmanager
    def plugin_container_filesystem(
    ...
    def plugin_source_artifacts(
        ...
        self.source.collect_build_artifacts(
            source_artifacts_dir, self.source_artifacts_dir
        )
        ...
        config.add_bind(
            source_artifacts_dir, guest_source_artifacts_dir, BindType.READONLY
        )
        ...
        # In build, run a script to acquire source artifacts into
        # /srv/moncic-ci/source
        copy_script = Script("Copy artifacts to build location")
        copy_script.run_unquoted(
            "cp -r --reflink=auto /srv/moncic-ci/source-artifacts/*"
            " /srv/moncic-ci/source/"
        )
    ...
    def container_config(self) -> Generator[ContainerConfig]:
        ...
        with contextlib.ExitStack() as stack:
            for plugin in self.plugins:
                log.info(
                    "Build plugin: %s",
                    (plugin.__doc__ or plugin.__name__).strip(),
                )
    ...
    def collect_artifacts_script(self) -> Script:
        ...
        """
        return Script(
            title="Collect artifacts produced inside the container",
            user=UserConfig.root(),
        )
