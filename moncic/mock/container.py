...
class MockContainer(Container):
    ...
    def _container(self) -> Generator[None, None, None]:
        with self.image.session.run_log.push(
            f"{self.image.name}: run container"
        ):
            yield None
    ...
    def run(
        ...
        # self.image.images.session.mock_log(system=self.image.name,
        # action="run", config=run_config, cmd=command)
    ...
    def run_shell(
        ...
        return subprocess.CompletedProcess(
            ["shell"], stdout=b"", stderr=b"", returncode=0
        )
